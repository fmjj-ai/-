        ...
        try {
          ...
          if (index !== -1) {
            this.tasks[index] = {
              ...
              result: data.result ?? this.tasks[index].result,
              error_msg: data.error ?? data.error_msg ?? this.tasks[index].error_msg
            }
          } else {
            this.tasks.unshift({
              id: data.task_id,
              ...
              progress: data.progress,
              result: data.result,
              error_msg: data.error ?? data.error_msg,
              name: data.name || '未命名任务',
              type: data.type || 'unknown',
              created_at: new Date().toISOString()
            })
          }
          if (!this.showTaskCenter && TERMINAL_TASK_STATUSES.has(data.status)) {
            this.unreadCount++
          }
        } catch (e) {
